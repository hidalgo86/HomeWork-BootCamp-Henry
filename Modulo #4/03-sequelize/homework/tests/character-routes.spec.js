...
describe('Character Routes', () => {
  ...
  describe('Parte UNO: POST /character', () => {
    it('debe devolver el estado 404 y el texto correspondiente si alguno de los parámetros obligatorios no se envía', async () => {
      const res = await request(app).post('/character');
      expect(res.statusCode).toBe(404);
      expect(res.text).toBe('Falta enviar datos obligatorios');
    });
    it('Debería devolver el estado 201 y el objeto de caracteres si el personaje se creó con éxito', async () => {
      const date = new Date();
      ...
      expect(res.body).toEqual({
        ...
        mana: 120.0,
        age: null,
        date_added: `${date.getFullYear()}-0${(date.getMonth() + 1)}-${date.getDate()}`,
        race: 'Other'
      });
    });
    it('debe devolver el estado 404 y el texto correspondiente si la creación de la base de datos falla', async () => {
      const res = await request(app)
                          .post('/character')
  ...
  describe('Multiple routes', () => {
    ...
    describe('Parte UNO', () => {
      it('debe devolver el estado 200 y la lista de todos los personajes', async () => {
        const date = new Date();
        const res = await request(app).get('/character');
        expect(res.statusCode).toBe(200);
        expect(res.body).toEqual([
          {code: 'ONE', name: 'First', hp: 90.0, mana: 150.0, age: 27, date_added: `${date.getFullYear()}-0${(date.getMonth() + 1)}-${date.getDate()}`, race: 'Human'},
          {code: 'TWO', name: 'Second', hp: 135.0, mana: 40.0, age: 20, date_added: `${date.getFullYear()}-0${(date.getMonth() + 1)}-${date.getDate()}`, race: 'Machine'},
          {code: 'THREE', name: 'Third', hp: 110.0, mana: 110.0, age: 23, date_added: `${date.getFullYear()}-0${(date.getMonth() + 1)}-${date.getDate()}`, race: 'Human'}
        ])
      })
      it('EXTRA: debe devolver el estado 200 y la lista de todos los caracteres con ciertos atributos', async () => {
      ...
      it('Debería enumerar todos los personajes que coincidan con el filtro de race', async () => {
      ...
      it('Debe devolver el estado 404 y el mensaje correcto si el código del personaje no es válido', async () => {
      ...
      it('Debería devolver la búsqueda de personaje correcta por código', async () => {
        const date = new Date();
